...
const ResultsManagement = () => {
  ...
  }, [tabValue, statusFilter, roomFilter]);
  ...
  const handleEditMarks = (student) => {
    ...
    setEditMarksValue(student.examMarks || '');
  ...
  const handleGenerateRanks = async () => {
    ...
    try {
      ...
      if (draftCheck.data.success && draftCheck.data.data.students?.length > 0) {
        toast.error(`Cannot generate ranks: ${draftCheck.data.data.students.length} students still have draft marks. Please ensure all marks are submitted first.`);
        return;
      }
    } catch (error) {
  ...
  const getStatusChip = (status) => {
    switch (status) {
      ...
      case 'draft':
        return <Chip label="Draft" color="info" size="small" icon={<EditIcon />} />;
      case 'submitted':
        return <Chip label="Submitted" color="success" size="small" icon={<CheckCircleIcon />} />;
      case 'final':
        return <Chip label="Final" color="secondary" size="small" icon={<LockIcon />} />;
      default:
    ...
    <Box>
      ...
      <Paper sx={{ mb: 4 }}>
        <Tabs value={tabValue} onChange={(e, v) => setTabValue(v)} sx={{ borderBottom: 1, borderColor: 'divider' }}>
          <Tab label="Top Performers" icon={<MedalIcon />} iconPosition="start" />
          <Tab label="Mark Management" icon={<EditIcon />} iconPosition="start" />
        </Tabs>
      </Paper>
              ...
              <Grid item xs={12} sm={4}>
                <FormControl fullWidth size="small">
                  ...
                  >
                    ...
                    {rooms.map((room) => (
                      <MenuItem key={room} value={room}>Room {room}</MenuItem>
                    ))}
                  </Select>
                </FormControl>
              </Grid>
                            ...
                            {student.markEntryStatus === 'submitted' && (
                              <Tooltip title="Finalize Marks">
                                <IconButton
                                  size="small"
                                  color="success"
                                  onClick={() => handleFinalizeMarks(student._id)}
      ...
      {/* Scholarship Winners */}
      {topPerformers.filter(s => s.scholarship).length > 0 && tabValue === 0 && (
        <Paper sx={{ mb: 4 }}>
          <Box sx={{ p: 3, borderBottom: '1px solid #e0e0e0' }}>
            <Typography variant="h6" fontWeight={600}>
              Scholarship Winners
      ...
      {/* Edit Marks Dialog */}
      <Dialog open={editMarksDialog} onClose={() => setEditMarksDialog(false)} maxWidth="sm" fullWidth>
        <DialogTitle>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <EditIcon color="primary" />
            Edit Marks - {selectedStudent?.name}
          </Box>
        </DialogTitle>
        <DialogContent>
          <Alert severity="info" sx={{ mb: 3 }}>
            Current Status: {selectedStudent?.markEntryStatus}
          </Alert>
          <TextField
            fullWidth
        ...
        <DialogActions>
          ...
          <Button
            ...
            startIcon={savingMarks ? <CircularProgress size={20} /> : <SaveIcon />}
          >
          ...
          {historyLoading ? (
            ...
            <>
              <Box sx={{ mb: 3 }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  Registration: {studentHistory?.student?.registrationCode}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  Current Marks: {studentHistory?.student?.currentMarks} | Status: {studentHistory?.student?.currentStatus}
                </Typography>
              </Box>
              <Divider sx={{ mb: 3 }} />
                        ...
                        <TableRow key={index}>
                          ...
                          <TableCell>
                            <Chip
                              ...
                              size="small"
                              color={
                                entry.action === 'entered' ? 'default' :
                                entry.action === 'updated' ? 'info' :
                                entry.action === 'submitted' ? 'success' :
                                'secondary'
                              }
                            />
                          </TableCell>
                          ...
                          <TableCell>
                            <Chip
                              ...
                              size="small"
                              color={
                                entry.status === 'draft' ? 'info' :
                                entry.status === 'submitted' ? 'success' :
                                entry.status === 'final' ? 'secondary' :
                                'default'
                              }
                            />
                          </TableCell>
