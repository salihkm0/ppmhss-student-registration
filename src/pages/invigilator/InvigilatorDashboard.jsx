...
import StudentList from "../../components/invigilator/StudentList.jsx";
import InvigilatorProfile from "../../components/invigilator/InvigilatorProfile.jsx";
import MarkHistory from "../../components/invigilator/MarkHistory.jsx";
import PendingSubmissions from "../../components/invigilator/PendingSubmissions.jsx";
...
const InvigilatorDashboard = () => {
    ...
    const fetchInvigilatorData = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          const newNotifications = [];
          response.data.dashboard.assignedRooms.forEach(room => {
            if (room.marksDraft > 0) {
              newNotifications.push({
                id: `draft-${room.roomNo}`,
                message: `Room ${room.roomNo}: ${room.marksDraft} marks in draft`,
                time: "Just now",
                type: "info",
                read: false,
              });
            }
            if (room.marksSubmitted > 0) {
              newNotifications.push({
                id: `submitted-${room.roomNo}`,
                message: `Room ${room.roomNo}: ${room.marksSubmitted} marks submitted`,
                time: "Today",
                type: "success",
                read: true,
              });
  ...
  const unreadNotifications = notifications.filter(n => !n.read).length;
  const pendingMarks = dashboardData?.stats?.totalMarksPending || 0;
  const draftMarks = dashboardData?.stats?.totalMarksDraft || 0;
  const submittedMarks = dashboardData?.stats?.totalMarksSubmitted || 0;
  const totalRooms = dashboardData?.stats?.totalAssignedRooms || 0;
          ...
          <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
            ...
            {draftMarks > 0 && (
              <a
                ...
                className="flex items-center px-4 py-3 text-sm font-medium text-blue-700 rounded-lg bg-blue-50 hover:bg-blue-100 hover:text-blue-800 border-l-4 border-blue-500"
              >
                <div className="mr-3">
                  <AssignmentIcon className="w-5 h-5" />
                </div>
                Draft Marks
                <span className="ml-auto px-2 py-1 text-xs font-bold bg-blue-500 text-white rounded-full">
                  {draftMarks}
                </span>
              </a>
            )}
            ...
            <div className="flex items-center space-x-4">
              ...
              {draftMarks > 0 && (
                <div className="hidden px-3 py-1 text-sm border border-blue-200 rounded-full md:flex items-center bg-blue-50">
                  <AssignmentIcon className="w-4 h-4 mr-1 text-blue-600" />
              ...
              {/* Submitted Marks Badge */}
              {submittedMarks > 0 && (
                <div className="hidden px-3 py-1 text-sm border border-green-200 rounded-full md:flex items-center bg-green-50">
                  <CheckCircleIcon className="w-4 h-4 mr-1 text-green-600" />
                  <span className="font-medium text-green-700">
                    {submittedMarks} submitted
                  </span>
                </div>
              )}
        ...
        <main className="px-4 py-6 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto">
            ...
            <Routes>
              <Route path="/dashboard" element={<InvigilatorHome dashboardData={dashboardData} />} />
              <Route path="/rooms" element={<AssignedRooms dashboardData={dashboardData} />} />
              <Route path="/marks" element={<EnterMarks dashboardData={dashboardData} onDataUpdate={() => {
                // Refresh dashboard data when marks are updated
              ...
              <Route path="/students" element={<StudentList dashboardData={dashboardData} />} />
              <Route path="/profile" element={<InvigilatorProfile invigilatorData={invigilatorData} />} />
              <Route path="/pending" element={<PendingSubmissions dashboardData={dashboardData} />} />
              <Route path="/history/:studentId" element={<MarkHistory />} />
              <Route path="/" element={<InvigilatorHome dashboardData={dashboardData} />} />
            </Routes>
          </div>
        </main>
