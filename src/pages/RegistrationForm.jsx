...
const RegistrationForm = () => {
  ...
  const fetchNextApplicationNo = async () => {
    try {
      ...
      const [appNoResponse, regCodeResponse] = await Promise.all([
        axiosInstance.get("/students/next-application-no"),
        axiosInstance.get("/students/next-registration-code"),
      ]);
  ...
  const handleNext = async () => {
    ...
    if (activeStep === 0) {
      // Must confirm eligibility first
      ...
      fields = ["schoolName", "subDistrict", "studyingClass", "medium"];
    } else {
      fields = [
        "phoneNo",
        "address.houseName",
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const response = await axiosInstance.post("/students/register", data);
            ...
            >
              <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1.5 }}>
                ...
                <Typography variant="subtitle1" fontWeight={700} sx={{ letterSpacing: 0.5 }}>
                  NMEA TENDER SCHOLAR — Eligibility
                </Typography>
              </Box>

              <Typography variant="body2" sx={{ opacity: 0.92, mb: 1.5, lineHeight: 1.6 }}>
                This scholarship examination is exclusively for students who have achieved
              ...
              <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, mb: 2 }}>
                {["Class 10 (SSLC)", "Class +2 (HSE)"].map((cls) => (
                  <Chip
                    key={cls}
              ...
              <Box
                sx={{
                  bgcolor: eligibilityError
                    ? "rgba(244,67,54,0.18)"
                    : eligibilityConfirmed
                    ? "rgba(76,175,80,0.18)"
                    : "rgba(255,255,255,0.08)",
                  border: eligibilityError
              ...
              >
                  ...
                  control={
                    <Checkbox
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.checked) setEligibilityError(false);
                      }}
                      sx={{
                        color: eligibilityError ? "#f44336" : "rgba(255,255,255,0.7)",
                        "&.Mui-checked": { color: "#69f0ae" },
                        py: 0.5,
                      }}
                      id="eligibility-confirm-checkbox"
                    />
                  }
                  label={
                    <Typography
                      variant="body2"
                      sx={{
                        color: eligibilityError
                          ? "#ff8a80"
                          : eligibilityConfirmed
                          ? "#69f0ae"
                          : "rgba(255,255,255,0.85)",
                        fontWeight: eligibilityConfirmed ? 600 : 400,
                        fontSize: "0.82rem",
                        lineHeight: 1.5,
                      }}
                    >
                      I confirm that the student has scored{" "}
                      <strong>A+ in ALL subjects</strong> and is eligible to appear
                ...
                {eligibilityError && (
                  <Typography
                    variant="caption"
                    sx={{ color: "#ff8a80", display: "block", pl: 4, mt: 0.25 }}
                  >
                    ⚠ You must confirm eligibility before proceeding.
                  </Typography>
                )}
              </Box>
            </Box>
...
//                 If you have already registered, you can check your application status below.
//                 For any inquiries, please contact us through WhatsApp support.
//               </Typography>
//             </Box>
